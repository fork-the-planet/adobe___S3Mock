...
* Features and fixes
  ...
  * chore: Modernized the Docker image build. The hand-written multi-stage `Dockerfile` and `docker buildx` shell scripts (and the entire `docker` module) were replaced by the Spring Boot Maven plugin's `build-image` goal, producing an OCI image via Cloud Native Buildpacks using Spring Boot's default multi-architecture Paketo `noble-java-tiny` builder. Regular builds produce a local-architecture image (matching the host, still skippable with `-DskipDocker`); releases build `linux/amd64` and `linux/arm64` and publish a merged multi-architecture manifest to Docker Hub. No change to the published image name or tags (`adobe/s3mock:<version>`, major, minor, `latest`).
* Version updates (deliverable dependencies)
...
* Features and fixes
  ...
  * feat: Experimental S3 Vectors API support — enable with `SPRING_PROFILES_ACTIVE=vectors`; runs on dedicated HTTP port 9092 / HTTPS port 9193. Supports `CreateVectorBucket`, `GetVectorBucket`, `ListVectorBuckets`, `DeleteVectorBucket`, `CreateIndex`, `GetIndex`, `ListIndexes`, `DeleteIndex`, `PutVectors`, `GetVectors`, `ListVectors`, `DeleteVectors`, `QueryVectors`, `PutVectorBucketPolicy`, `GetVectorBucketPolicy`, `DeleteVectorBucketPolicy`, `TagResource`, `UntagResource`, `ListTagsForResource`. See [README.md#s3-vectors-support](README.md#s3-vectors-support).
    ...
    * [ITs](integration-tests/src/test/kotlin/com/adobe/testing/s3mock/its/vectors) pass against the real S3 API except the [VectorPolicyIT](integration-tests/src/test/kotlin/com/adobe/testing/s3mock/its/vectors/VectorPolicyIT.kt) which would need real AWS principals.
