 ...
 * mapped to [com.adobe.testing.s3mock.s3.dto.ErrorResponse] and serialized.
...
class S3Exception(
  ...
  companion object {
    ...
    val INVALID_PART_ORDER: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        "InvalidPartOrder",
    ...
    fun completeRequestMissingChecksum(
      algorithm: String,
      partNumber: Int,
    ): S3Exception =
      S3Exception(
    ...
    val ENTITY_TOO_SMALL: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        "EntityTooSmall",
    ...
    val NO_SUCH_BUCKET: S3Exception =
      S3Exception(
        HttpStatus.NOT_FOUND.value(),
        "NoSuchBucket",
        "The specified bucket does not exist.",
      )
    val NO_SUCH_LIFECYCLE_CONFIGURATION: S3Exception =
      S3Exception(
        HttpStatus.NOT_FOUND.value(),
    ...
    val NO_SUCH_VERSION: S3Exception =
      S3Exception(
        HttpStatus.NOT_FOUND.value(),
        "NoSuchVersion",
        "The version ID specified in the " +
    ...
    val PRECONDITION_FAILED: S3Exception =
      S3Exception(
        HttpStatus.PRECONDITION_FAILED.value(),
        "PreconditionFailed",
        "At least one of the pre-conditions you specified did not hold",
      )
    val BUCKET_NOT_EMPTY: S3Exception =
      S3Exception(
        HttpStatus.CONFLICT.value(),
        "BucketNotEmpty",
        "The bucket you tried to delete is not empty.",
      )
    val INVALID_BUCKET_NAME: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        "InvalidBucketName",
        "The specified bucket is not valid.",
      )
    val BUCKET_ALREADY_EXISTS: S3Exception =
      S3Exception(
        HttpStatus.CONFLICT.value(),
        "BucketAlreadyExists",
        (
    ...
    val BUCKET_ALREADY_OWNED_BY_YOU: S3Exception =
      S3Exception(
        HttpStatus.CONFLICT.value(),
        "BucketAlreadyOwnedByYou",
        "Your previous request to create the named bucket succeeded and you already own it.",
      )
    val NOT_FOUND_BUCKET_OBJECT_LOCK: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        INVALID_REQUEST_CODE,
        "Bucket is missing Object Lock Configuration",
      )
    val NOT_FOUND_BUCKET_VERSIONING_CONFIGURATION: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        INVALID_REQUEST_CODE,
        "Bucket is missing Versioning Configuration",
      )
    val NOT_FOUND_OBJECT_LOCK: S3Exception =
      S3Exception(
        HttpStatus.NOT_FOUND.value(),
        "NotFound",
        "The specified object does not have a ObjectLock configuration",
      )
    val INVALID_REQUEST_RETAIN_DATE: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        INVALID_REQUEST_CODE,
        "The retain until date must be in the future!",
      )
    val INVALID_REQUEST_MAX_KEYS: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        INVALID_REQUEST_CODE,
    ...
    val INVALID_REQUEST_MAX_BUCKETS: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        INVALID_REQUEST_CODE,
        "maxBuckets should be positive",
      )
    val INVALID_REQUEST_ENCODING_TYPE: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        INVALID_REQUEST_CODE,
        "encodingtype can only be none or 'url'",
      )
    val INVALID_COPY_REQUEST_SAME_KEY: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        INVALID_REQUEST_CODE,
        (
    ...
    val BAD_REQUEST_MD5: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        BAD_REQUEST_CODE,
        "Content-MD5 does not match object md5",
      )
    val BAD_REQUEST_CONTENT: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        "UnexpectedContent",
        "This request contains unsupported content.",
      )
    val BAD_DIGEST: S3Exception =
      S3Exception(
        HttpStatus.BAD_REQUEST.value(),
        "BadDigest",
