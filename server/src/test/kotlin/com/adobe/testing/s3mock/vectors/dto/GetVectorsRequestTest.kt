...
internal class GetVectorsRequestTest {
  @Test
  fun testDeserialization(testInfo: TestInfo) {
    val iut = DtoTestUtil.deserializeJSON(GetVectorsRequest::class.java, testInfo)

    assertThat(iut.vectorBucketName).isEqualTo("my-vector-bucket")
    assertThat(iut.indexName).isEqualTo("my-index")
    assertThat(iut.keys).containsExactly("doc-1", "doc-2", "doc-3")
